...
class self_encryption_decryption_inf_sharing:
    ...
    def encodeWithPadding(self, plaintext, password):
        ...
        iv = get_random_bytes(16)  # Twofish uses a 128-bit (16-byte) IV for CBC mode
        key = PBKDF2(password, salt, 32, count=1000000, hmac_hash_module=SHA256)  # Derive a 256-bit key (32 bytes)
        
        cipher = Twofish(key)  # Initialize the Twofish cipher with the derived key
        
        # Ensure the plaintext is padded to a multiple of the block size (16 bytes for Twofish)
        padded_plaintext = pad(plaintext, 16)
        
        # Encrypt in CBC mode manually
        ciphertext = b""
        previous_block = iv  # Initial block for CBC is the IV
        for i in range(0, len(padded_plaintext), 16):
            block = padded_plaintext[i:i+16]  # Get the next 16-byte block
            # block_to_encrypt = xor_blocks(block, previous_block)  # XOR with the previous block (or IV for first block)
            block_to_encrypt = bytes([b1 ^ b2 for b1, b2 in zip(block, previous_block)])
            encrypted_block = cipher.encrypt(block_to_encrypt)  # Encrypt the block
            ciphertext += encrypted_block  # Append the encrypted block to the ciphertext
            previous_block = encrypted_block  # Update the previous block to the current encrypted block
    
        return ciphertext + salt + iv  # Append salt and IV for storage

    # Decoding method, takes the encrypted data and decrypts it using a password
    def decodeWithPadding(self, data, password):
        salt = data[-32:-16]  # Extract the 16-byte salt (last 32-16 bytes)
        key = PBKDF2(password, salt, 32, count=1000000, hmac_hash_module=SHA256)  # Derive the key
        iv = data[-16:]  # Extract the 16-byte IV (last 16 bytes)
        
        cipher = Twofish(key)  # Initialize the Twofish cipher with the derived key
        ciphertext = data[:-32]  # Extract the ciphertext (everything except salt and IV)
        
        # Decrypt in CBC mode manually
        decrypted_message = b""
        previous_block = iv  # Initial block for CBC is the IV
        
        # Process each 16-byte block
        for i in range(0, len(ciphertext), 16):
            block = ciphertext[i:i+16]  # Get the next 16-byte block
            decrypted_block = cipher.decrypt(block)  # Decrypt the block
            decrypted_block = bytes([b1 ^ b2 for b1, b2 in zip(decrypted_block, previous_block)])  # XOR with the previous block
            decrypted_message += decrypted_block  # Append to the decrypted message
            previous_block = block  # Update the previous block to the current ciphertext block
        
        try:
            # Unpad the decrypted message to its original size
